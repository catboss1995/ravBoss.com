...
class AnnouncementService {
  ...
  async getAnnouncements() {
    try {
      ...
      // 如果API請求失敗，返回默認數據並處理圖片
      return this.defaultAnnouncements.map(announcement => ({
        ...announcement,
        image: announcement.image.startsWith('http') 
          ? announcement.image  // 如果已經是網路URL，直接使用
          : this.getAnnouncementImage(announcement.image) // 否則從本地圖片獲取
      }));
    }
  }
